...
class ErrorSpec extends Specification with IOMatchers with SomeErrors { def is: SpecStructure =
  s2"""
  ...
  def manageConflict: MatchResult[IO[String]] =
    effectWithNotFound[IO].inConflict("someString".pure[IO]) must returnValue("someString")
}
