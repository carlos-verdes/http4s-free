  ...
  def arangoResourceInterpreter(clientR: Resource[IO, Arango[IO]]): ResourceAlgebra ~> IO = new (ResourceAlgebra ~> IO) {
    ...
    override def apply[A](op: ResourceAlgebra[A]): IO[A] = (op match {
      ...
      case LinkResources(leftUri, rightUri, relType) =>
        ...
        println(s" testing edge ${leftUri.path.toString()}")
        println(s" ${edgeDocument.values}")

        withEdge(relType)(_.documents.insert(document = edgeDocument, overwrite = true, returnNew = true))
            .map(_ => ().resultOk)
            //withEdge(relType)(_.documents.insert(GraphEdgeDefinition(relType, List(leftCol), List(rightCol))))
